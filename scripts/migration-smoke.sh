...
public_privilege_count="$(docker compose exec -T db psql --username postgres --dbname "$database_name" --tuples-only --no-align --command "SELECT count(*) FROM pg_roles role CROSS JOIN unnest(ARRAY['companies', 'jobs', 'hiring_signals', 'ingestion_runs', '_prisma_migrations']) AS table_name WHERE role.rolname IN ('anon', 'authenticated') AND (has_table_privilege(role.rolname, 'public.' || table_name, 'SELECT') OR has_table_privilege(role.rolname, 'public.' || table_name, 'INSERT') OR has_table_privilege(role.rolname, 'public.' || table_name, 'UPDATE') OR has_table_privilege(role.rolname, 'public.' || table_name, 'DELETE'))")"
